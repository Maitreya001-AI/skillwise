...
4. **Re-score through the shared gate (authoritative).** Run `../../shared/effect-gate.md` on the chosen edit: held-out **edited vs previous-accepted** (`delta_step`), reusing the once-computed per-task `no_skill` only for the floor check; re-run the reference (previous-accepted) condition **≥3×** to size the noise band (other conditions ≥2×); judges blind, median of ≥2. Only this run's `delta_step` decides KEEP. It yields the `gate` object — read `gate_pass`, and from its `effect` block `delta_step` / `delta_exist` / `floor_ok` / `regression_count` / `safety_regression`.
...
- **Lateral keep** (`delta_step` within the noise band — with no per-task regression vs previous, floor intact, and safety-clean) only if the edit *removes* a Tier-1 blocking finding; otherwise a within-noise change is a revert.
...
- **`improved`** — ≥1 edit kept, each beating the previous version beyond the noise band; report the cumulative `delta_step` gain.
...
- **`cannot_measure`** — the **harness itself** couldn't produce scores: no executor, no obtainable held-out set (the gate's `static_only`), or scoring errored out. The null result is the harness's fault, not the skill's: do **not** report success. Escalate with the remedy — supply/repair the held-out set, or route to `seek-skill` for a structural rethink.

These are mutually exclusive: `unfit_test_set` (current skill maxed, no negative transfer) routes to `already_optimal` (consistent with `effect-gate.md` and the ratchet), **not** `cannot_measure` — which is reserved for a genuinely unrunnable harness.

Stop also on max rounds or user stop. A version carrying negative transfer (floor breach) or a safety regression never counts as done — fall back to the last clean KEEP. **If no clean KEEP exists and the surviving (entry) version itself breaches the floor (no KEEP ⇒ the tree was reverted to entry) — it loses to no-skill and no repair landed — the run did *not* reach a done state: the repair failed. Escalate (route to `seek-skill` for a structural rethink, or supply a stronger fix); never report `already_optimal` for a sub-floor skill.** A run the user asked to improve that ends with ~0 change must therefore resolve to one of: `already_optimal` (floor-clean, no measurable gain), `cannot_measure` (harness unrunnable), or the failed-repair escalation above — never an unqualified "done".
