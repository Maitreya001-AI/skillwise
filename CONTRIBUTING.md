...
4. **Clear the gate.** For a *new* skill, show it beats the no-skill baseline beyond the noise band (`delta_exist > noise_band_exist`): run representative tasks with and without it. For an *edit to an existing* skill, show it beats the skill's *previous version* (`delta_step` beyond the noise band) while never dropping below the no-skill floor — see [`shared/effect-gate.md`](./shared/effect-gate.md) (the single editing source; it is materialized into each consumer skill's `references/` by `scripts/sync-shared.py`, which CI enforces). The measurement is one local command, not a hand-built harness: write a `tasks.yaml` (schema in [`shared/scripts/gate_runner.py`](./shared/scripts/gate_runner.py)) and run `python shared/scripts/gate_runner.py run <skill>/tasks.yaml` — arms, isolation, per-run cost, and the arithmetic are all mechanical. A version worse than its reference is the one defect we never merge.
