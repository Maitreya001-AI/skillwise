...
Stop when the diverse pool yields no new pattern that passes the held-out gate, or the corpus is exhausted. A candidate that can't beat the no-skill baseline is not a skill — report "no gap here," don't ship a macro.

## Output

Per generated skill: the gap it fills (with the recurring evidence), its type, the held-out with/without delta, and the dedup decision. Show the discarded candidates — patterns that looked promising but failed the gate are as informative as the kept ones.
