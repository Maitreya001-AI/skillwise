...
> **Update (2026-07 — the gate's mechanics are compiled).** The effect tier is no longer hand-run: `shared/scripts/gate_runner.py` + `gate_math.py` (materialized into each consumer's `scripts/` by the same sync) execute the protocol as one command — isolated with/without sandboxes, product assertions, per-run cost capture, and the §8 arithmetic. Wherever this note implies a hand-assembled measurement, read the runner; `evaluate-skill`'s Tier 1 now also drafts the `tasks.draft.yaml` the runner consumes.
