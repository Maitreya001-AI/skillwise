...
**Most skills are composites.** Name the atoms first: a domain/ontology skill (e.g. a DDD modeling skill, a spreadsheet skill) is Knowledge + Capability + Judgment; an extraction/pipeline skill is Control + Judgment + Capability. For a composite, the human checkpoint goes at the **coverage boundary of the declared judgment criteria** (the seam theorem, §6): the machine judges everything the declared φ covers — that is what shipping a Judgment atom *means* — and the human owns only the **residual** beyond it (taste, machine-unjudgeable semantic quality). Realized concretely as the `done_when`/role-separation boundary. Misplacement fails in both directions: the machine adjudicating the residual (over-fill), or the human inserted where the declared criteria already decide (under-automation).
