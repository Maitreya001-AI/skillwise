...
**Deletion, compiled — run the probe, never simulate (the §4 form theorem applied to this skill's own weakest judgment).** For any style/convention/best-practice skill, the deletion test's counterfactual — *would a capable unaided engine already produce these rules?* — must come from an observation: run `python3 scripts/deletion_probe.py <skill-dir>` (ships with this skill). It generates the unaided bar for the skill's domain in an isolated engine call that never sees the skill's body, then prints both texts with a classification scaffold. Classify each rule against the concrete unaided bar: zero substantive survivors → `nogap`; any substantive survivor (a threshold, a grammar, a named selection stated as enforced) defeats a `nogap` conviction. If the probe cannot run (exit 3), emit the deletion finding with `prediction: true` — never fake its output. The probe feeds the deletion test only; every other test and the pricing clause below run unchanged.
