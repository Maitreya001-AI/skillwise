...
|---|---|---|---|
| evaluate-skill | production | checked (linter clean; adversarially reviewed) | **`fail` (r2 existence run, 2026-07) — negative transfer, now localized and in escalated repair.** The r2 run: +0.17 broken-skill detection, −0.25 good-skill preservation, gap real (`no_skill` 0.75). Four improve rounds followed (see below): the working set was hardened until the defect expressed reproducibly (29 seeds, band 0.0398), which **localized the negative transfer to the decoy class** (generic content in org costume; rhetoric-justified order) — and all three repair mechanisms were mechanically REVERTed by the gate, one failure mode each (prose under-transmits 1/3 runs; a forced default over-convicts boundary goods; a pre-registration micro-probe displaces attention with 0.759–0.966 run variance). The loop **stopped on evidence at 4/10 rounds** with two structural findings: the boundary *is* the deletion test (L2-native, §7) so the next form is a compiled `scripts/` probe that actually runs a no-skill engine; and nogap-class seed labels must first be re-established behaviorally — they are by-construction assertions on the very axis under test. See [`improve-run-evaluate-skill/round-4.md`](./improve-run-evaluate-skill/round-4.md). |
